...
ARD_LIGHT_SENSOR_TOPIC = os.getenv("ARD_LIGHT_SENSOR_TOPIC")
ARD_LIGHT_ACTUATOR_TOPIC = os.getenv("ARD_LIGHT_ACTUATOR_TOPIC")
ARD_LIGHT_PORT = os.getenv("ARD_LIGHT_PORT")
...
def read_data_t():
    while True:
        ...
        try:
            ...
            logging.info(f"Received data: {data_string} from {ARD_LIGHT_PORT}")
...
def main():
    ...
    local_mqtt_client.message_callback_add(ARD_LIGHT_ACTUATOR_TOPIC, change_water_pump_threshold)
    
    # for processing control messages from the flask server
    local_mqtt_client.connect(LOCAL_INTERFACE_IP, 1883)
    local_mqtt_client.subscribe(ARD_LIGHT_ACTUATOR_TOPIC)
